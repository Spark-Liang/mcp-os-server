...
from mcp_os_server.command.interfaces import ProcessStatus
...
@pytest_asyncio.fixture
async def executor(process_manager: ProcessManager) -> AsyncGenerator[CommandExecutor, None]:
...
@pytest.mark.asyncio
async def test_execute_command_end_to_end(executor: CommandExecutor, tmp_path: Path):
    """端到端命令执行测试"""
...
@pytest.mark.asyncio
async def test_execute_command_timeout(executor: CommandExecutor, tmp_path: Path):
    """同步命令超时测试"""
...
@pytest.mark.asyncio
async def test_execute_command_uninterruptible_timeout(executor: CommandExecutor, tmp_path: Path):
    """测试不可中断的死循环命令也能被超时控制"""
...
@pytest.mark.asyncio
async def test_execute_command_with_specific_exit_code(executor: CommandExecutor, tmp_path: Path):
    """测试命令返回特定退出码"""
...
@pytest.mark.asyncio
async def test_execute_command_with_stdin(executor: CommandExecutor, tmp_path: Path):
    """测试带有stdin的命令执行"""
...
@pytest.mark.asyncio
async def test_background_command_lifecycle(executor: CommandExecutor, tmp_path: Path):
    """后台任务生命周期测试"""
...
@pytest.mark.asyncio
async def test_concurrent_background_commands(executor: CommandExecutor, tmp_path: Path):
    """并发后台命令测试"""
    commands = [
...
@pytest.mark.asyncio
async def test_command_execution_error_for_nonexistent_command(executor: CommandExecutor, tmp_path: Path):
...
@pytest.mark.asyncio
async def test_cleanup_finished_process(executor: CommandExecutor, tmp_path: Path):
    """测试清理已完成的进程"""
...
@pytest.mark.asyncio
async def test_encoding_support(executor: CommandExecutor, tmp_path: Path):
    """环境变量与编码支持测试"""
    # Test gbk encoding
    text_gbk = "你好世界"
...
@pytest.mark.asyncio
async def test_shutdown_stops_running_processes(tmp_path: Path):
    """测试关闭时能停止所有正在运行的进程"""
...
@pytest.mark.asyncio
async def test_command_pipe_simulation(executor: CommandExecutor, tmp_path: Path):
    """测试模拟管道功能 (cmd1 | cmd2)"""
...
@pytest.mark.asyncio
async def test_concurrent_stop_and_clean(executor: CommandExecutor, tmp_path: Path):
    """测试并发停止和清理进程"""
...
@pytest.mark.asyncio
async def test_path_lookup_functionality(executor: CommandExecutor, tmp_path: Path):
...
@pytest.mark.asyncio
async def test_npm_path_lookup_optional(executor: CommandExecutor, tmp_path: Path):
...
@pytest.mark.asyncio
async def test_cmd_script_execution(executor: CommandExecutor, tmp_path: Path):
    """测试 .CMD 脚本文件执行功能"""
    # 获取测试 CMD 文件的路径
    test_cmd_file = Path(__file__).parent / "test_cmd_file.cmd"
...
@pytest.mark.asyncio
async def test_npm_debug_execution(executor: CommandExecutor, tmp_path: Path):
    """调试测试：专门检查 npm 执行情况"""
    import os
...
@pytest.mark.asyncio
async def test_mcp_command_simulation(executor: CommandExecutor, tmp_path: Path):
