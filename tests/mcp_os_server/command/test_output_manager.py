...
async def test_store_and_get_output(output_manager):
    ...
    # 验证输出消息列表
    await output_manager.store_output(
        process_id, output_key_stdout, ["list_msg_1", "list_msg_2"]
    )
...
async def test_get_output_with_filters(output_manager):
    ...
    now = datetime.now()
    await output_manager.store_output(
        process_id, output_key, "line 1", timestamp=now - timedelta(seconds=10)
    )
    await output_manager.store_output(
        process_id, output_key, "line 2 keyword", timestamp=now - timedelta(seconds=5)
    )
    await output_manager.store_output(process_id, output_key, "line 3", timestamp=now)
    await output_manager.store_output(
        process_id,
        output_key,
        "another keyword line 4",
        timestamp=now + timedelta(seconds=5),
    )

    # Test 'since' filter
    output_since = []
    async for entry in output_manager.get_output(
        process_id, output_key, since=(now - timedelta(seconds=1)).timestamp()
    ):
    ...
    output_until = []
    async for entry in output_manager.get_output(
        process_id, output_key, until=(now + timedelta(seconds=1)).timestamp()
    ):
    ...
    output_grep_line = []
    async for entry in output_manager.get_output(
        process_id, output_key, grep_pattern="keyword"
    ):
    ...
    output_grep_content = []
    async for entry in output_manager.get_output(
        process_id, output_key, grep_pattern="keyword", grep_mode="content"
    ):
...
async def test_storage_error_handling(mocker, output_manager):
    ...
    output_manager._loggers[process_id] = {
        output_key: mock_logger
    }  # Assign as nested dict

    mocker.patch.object(
        mock_logger, "add_message", side_effect=StorageError("Simulated storage error")
    )
    mocker.patch.object(
        mock_logger, "add_messages", side_effect=StorageError("Simulated storage error")
    )
...
async def test_shutdown_error_handling(mocker, output_manager):
    ...
    mock_logger_instance_stdout = MagicMock(
        spec=SqliteOutputLogger
    )  # Use SqliteOutputLogger
    mock_logger_instance_stderr = MagicMock(
        spec=SqliteOutputLogger
    )  # Need another mock for stderr
    mock_logger_instance_stdout.close.side_effect = Exception(
        "Simulated stdout shutdown error"
    )
    mock_logger_instance_stderr.close.side_effect = Exception(
        "Simulated stderr shutdown error"
    )
...
async def test_timeout_process_output_retention(output_manager):
    ...
    # 存储超时进程的输出
    await output_manager.store_output(
        process_id, output_key_stdout, timeout_stdout_messages
    )
    await output_manager.store_output(
        process_id, output_key_stderr, timeout_stderr_messages
    )
    ...
    await output_manager.store_output(
        process_id, output_key_stdout, post_timeout_message
    )
...
async def test_concurrent_store_operations(output_manager):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=num_threads) as executor:
        ...
        for future in futures:
            if future.exception():
                pytest.fail(
                    f"Thread execution failed with exception: {future.exception()}"
                )
    ...
    assert (
        len(all_messages) == expected_total
    ...
    for thread_id in range(num_threads):
        for msg_id in range(messages_per_thread):
            ...
            assert (
                expected_message in all_messages
            ), f"Missing message: {expected_message}"
...
async def test_concurrent_read_write_operations(output_manager):
    ...
    with concurrent.futures.ThreadPoolExecutor(
        max_workers=num_writer_threads + num_reader_threads
    ) as executor:
        # 先提交写入线程
        writer_futures = [
            executor.submit(writer_thread, i) for i in range(num_writer_threads)
        ]
        ...
        # 然后提交读取线程
        reader_futures = [
            executor.submit(reader_thread, i) for i in range(num_reader_threads)
        ]
    ...
    # 验证没有线程错误
    assert (
        len(results["write_errors"]) == 0
    ), f"Write errors occurred: {results['write_errors']}"
    assert (
        len(results["read_errors"]) == 0
    ...
    assert (
        max(results["read_counts"]) <= expected_total
    ), "Read count exceeded expected total"
                ...
                if msg_index % 2 == 0:
                    ...
                    loop.run_until_complete(
                        output_manager.store_output(
                            process_id, "stderr", stderr_message
                        )
                    )
        finally:
            loop.close()
...
async def test_stress_concurrent_operations(output_manager):
    ...
    operation_results = {"success_count": 0, "error_count": 0, "errors": []}
    lock = threading.Lock()
    ...
    # 启动高并发操作
    with concurrent.futures.ThreadPoolExecutor(
        max_workers=num_concurrent_operations
    ) as executor:
        futures = [
    ...
    total_actual_operations = (
        operation_results["success_count"] + operation_results["error_count"]
    )

    assert (
        total_actual_operations == total_expected_operations
    ...
    print(
        f"Stress test completed: {operation_results['success_count']} successful operations"
    )
...
async def test_thread_safety_sqlite_connections(output_manager):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=num_threads) as executor:
        futures = [
            executor.submit(sqlite_thread_operation, i) for i in range(num_threads)
        ]
        ...
        for i, future in enumerate(futures):
            try:
                ...
                if (
                    ...
                    pytest.fail(
                        f"SQLite thread safety error detected in thread {i}: {error_msg}"
                    )
                else:
    ...
    assert (
        len(final_messages) == expected_total
    ), f"Expected {expected_total} messages, got {len(final_messages)} - data integrity issue"

    print(
        f"SQLite thread safety test passed: {len(final_messages)} messages stored and retrieved successfully"
    )
