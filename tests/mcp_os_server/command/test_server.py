...
async def mcp_server(process_manager: IProcessManager) -> FastMCP:
    ...
    define_mcp_server(
        ...
        ],  # Include all commands needed for tests
...
class TestMCPServerBasicFunctionality:
    ...
    async def test_command_ps_list_empty(self, mcp_server):
        ...
        result = await mcp_server.call_tool(
            "command_ps_list", {"labels": None, "status": None}
        )
        assert isinstance(result, list)
        assert len(result) == 1
        assert isinstance(result[0], TextContent)
    ...
    @pytest.mark.anyio
    async def test_command_ps_list_with_processes(
        self, mcp_server, process_manager, tmp_path
    ):
...
class TestMCPServerDefensiveProgramming:
    ...
    async def test_command_execute_timeout(self, mcp_server, tmp_path):
        ...
        assert (
            "Process" in result[3].text and "might still be running" in result[3].text
        )
    ...
    async def test_command_ps_list_invalid_status(self, mcp_server):
        ...
        result = await mcp_server.call_tool(
            "command_ps_list", {"labels": None, "status": "invalid_status"}
        )
        assert isinstance(result, list)
        assert len(result) == 1
        assert isinstance(result[0], TextContent)
    ...
    async def test_command_ps_stop_nonexistent_process(self, mcp_server):
        ...
        result = await mcp_server.call_tool(
            "command_ps_stop", {"pid": "nonexistent-pid", "force": False}
        )
        assert isinstance(result, list)
        assert len(result) == 1
        assert isinstance(result[0], TextContent)
    ...
    async def test_command_ps_detail_nonexistent_process(self, mcp_server):
        ...
        result = await mcp_server.call_tool(
            "command_ps_detail", {"pid": "nonexistent-pid"}
        )
        assert isinstance(result, list)
        assert len(result) == 1
        assert isinstance(result[0], TextContent)
    ...
    @pytest.mark.anyio
    async def test_command_ps_logs_no_logs_requested(
        self, mcp_server, process_manager, tmp_path
    ):
    ...
    async def test_command_ps_clean_empty_list(self, mcp_server):
        ...
        result = await mcp_server.call_tool("command_ps_clean", {"pids": []})
        assert isinstance(result, list)
        assert len(result) == 1
        assert isinstance(result[0], TextContent)
    ...
    @pytest.mark.anyio
    async def test_command_ps_list_filter_by_status(
        self, mcp_server, process_manager, tmp_path
    ):
        ...
        try:
            ...
            result = await mcp_server.call_tool(
                "command_ps_list", {"labels": None, "status": "running"}
            )
            assert isinstance(result, list)
            assert len(result) == 1
            assert isinstance(result[0], TextContent)
            ...
            result = await mcp_server.call_tool(
                "command_ps_list", {"labels": None, "status": "completed"}
            )
            assert isinstance(result, list)
            assert len(result) == 1
            assert isinstance(result[0], TextContent)
            # Should either be empty table or "No processes found"
            assert validate_process_list_table(
                result[0].text
    ...
    async def test_command_execute_with_custom_encoding(self, mcp_server, tmp_path):
        ...
        result = await mcp_server.call_tool(
            ...
            {
                "command": sys.executable,
                "args": [str(CMD_SCRIPT_PATH), "echo", "测试"],
                "directory": str(tmp_path),
                "stdin": None,
                "timeout": 15,
                "envs": None,
    ...
    @pytest.mark.anyio
    async def test_command_execute_with_environment_variables(
    ...
    @pytest.mark.anyio
    async def test_command_ps_logs_with_stderr(
        self, mcp_server, process_manager, tmp_path
    ):
...
class TestMCPServerConfigurationParameters:
    ...
    @pytest.fixture
    def mcp_server_with_restricted_commands(
        self, process_manager: AnyioProcessManager
    ) -> FastMCP:
        ...
        define_mcp_server(
            ...
            allowed_commands=[
                sys.executable
            ],  # Only allow the actual python executable
            default_encoding="utf-8",
            command_default_encoding_map={},
            default_timeout=15,
        )
    ...
    @pytest.fixture
    def mcp_server_with_custom_encoding(
        self, process_manager: AnyioProcessManager
    ) -> FastMCP:
    ...
    @pytest.mark.anyio
    async def test_allowed_commands_restriction(
        self, mcp_server_with_restricted_commands, tmp_path
    ):
    ...
    @pytest.mark.anyio
    async def test_allowed_commands_background_restriction(
        self, mcp_server_with_restricted_commands, tmp_path
    ):
    ...
    @pytest.mark.anyio
    async def test_default_encoding_usage(
        self, mcp_server_with_custom_encoding, tmp_path
    ):
    ...
    @pytest.mark.anyio
    async def test_process_retention_seconds(
