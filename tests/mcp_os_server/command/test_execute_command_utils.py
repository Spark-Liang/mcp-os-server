...
from mcp_os_server.command.models import CommandResult, ProcessStatus
from mcp_os_server.command.exceptions import CommandExecutionError, ProcessTimeoutError
from mcp_os_server.command.models import OutputMessageEntry
...
def mock_output_manager(mocker) -> IOutputManager:
    ...
    async def _mock_store_output(process_id: str, output_key: str, message: str | list[str]):
        ...
        for msg in messages_to_store:
            entry = OutputMessageEntry(timestamp=datetime.now(), text=msg, output_key=output_key)
            if process_id not in mock._stored_outputs:
                mock._stored_outputs[process_id] = []
            mock._stored_outputs[process_id].append(entry)

    async def _mock_get_output_impl(process_id: str, output_key: str, since: Optional[float] = None, until: Optional[float] = None, tail: Optional[int] = None) -> AsyncGenerator[OutputMessageEntry, None]:
        if process_id not in mock._stored_outputs:
            return
