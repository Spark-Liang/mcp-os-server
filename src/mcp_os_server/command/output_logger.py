...
class SqliteOutputLogger(IOutputLogger):
    ...
    def _get_connection(self):
        if self._conn is None:
            # Python's sqlite3 module handles Unicode by default.
            # No explicit encoding parameter is needed or supported by sqlite3.connect for the database file itself.
            self._conn = sqlite3.connect(self.log_file_path)
    ...
    def add_messages(self, messages: List[str], timestamp: Optional[datetime] = None) -> None:
        ...
        with conn:
            cursor = conn.cursor()
            current_timestamp = timestamp if timestamp else datetime.now(timezone.utc)
            data_to_insert = [
                (current_timestamp.timestamp(), message) for message in messages # Store as Unix timestamp
            ]
            cursor.executemany(f"INSERT INTO {self.table_name} (timestamp, text) VALUES (?, ?)", data_to_insert)
            conn.commit()
    ...
    async def get_logs(
        ...
        with conn:
            ...
            for row in cursor.fetchall():
                ...
                all_logs.append(OutputMessageEntry(timestamp=timestamp, text=row['text'], output_key=self.sub_id))
