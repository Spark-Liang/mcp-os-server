...
def try_resolve_win_path_in_url_format(path: str) -> Path:
    ...
    if sys.platform.startswith("win") and match:
        # 处理 /e:/... 格式的路径，转换为 e:/...
...
class RootInfoItem(BaseModel):
    ...
    local_path: Optional[Path] = Field(
        None,
        description="root 对应的本地路径，如果为 None，则表示 root 对应的本地路径无法获取。",
    )
...
async def list_roots(context: Context) -> List[RootInfoItem]:
    ...
    for root in roots.roots:
        if root.uri.scheme == "file":
            ...
            root_path_str = (
                urllib.parse.unquote(root.uri.path) if root.uri.path else "/"
            )
...
async def resolve_paths_and_check_allowed(
    ...
    async def resolve_relative_path_from_roots(request_path: Path) -> Optional[Path]:
        ...
        for root_info in root_infos:
            if not (root_info.local_path and root_info.local_path.is_absolute()):
                logger.debug(
                    "root %s local_path is not absolute, skip: %s",
                    root_info.root.name,
                    root_info.local_path,
                )
                continue
            ...
            logger.debug(
                "resolved_request_path %s not exists, skip: %s",
    ...
    for path in paths:
        ...
        request_path = (
            try_resolve_win_path_in_url_format(path) if isinstance(path, str) else path
        )
        ...
        if allowed_dirs is not None:
            for allowed in [Path(d) for d in allowed_dirs]:
                ...
                if is_request_absolute:
                    ...
                    resolved_result = await resolve_relative_path_from_roots(
                        request_path
                    )
                    if resolved_result:
            ...
            if resolved_path is None:
                raise PermissionError(
                    f"路径不在允许的目录中: {path}，allowed_dirs: {allowed_dirs}"
                )
        else:
            logger.debug("allowed_dirs is None, resolve relative path from roots")
            if is_request_absolute:
                logger.debug(
                    "request_path %s is absolute, return: %s",
                    request_path,
                    request_path,
                )
                resolved_path = request_path
                ...
                if resolved_result:
                    ...
                    logger.debug(
                        "request_path %s is not relative to any allowed_dirs, skip",
                        request_path,
                    )
